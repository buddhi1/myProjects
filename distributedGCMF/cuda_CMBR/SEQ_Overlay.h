...
int ReadTextFormatPolygon2(const char* fileName, int* bVNum, long* bVPSNum, mbr_t* xMBR, mbr_t *yMBR, 
                            coord_t* xMBR2, coord_t *yMBR2, coord_t* xCoords, coord_t *yCoords, 
                            long* bVNumSum, char mbrType, int maxPoly){
    int fNum=0, maxV=0;
    FILE *fid1;
    char tkBuff[2][50];
    char* buff=(char*)malloc(sizeof(char)*MAX_BUFF);
    ...
    while(fgets(buff,MAX_BUFF,fid1)!=NULL && pCounter<maxPoly){        
        ...
        //x1
        xMBR[2 * pCounter]=CoordToMBR(cX1, mbrType);
        xMBR2[2 * pCounter]=atof(cX1);
	//y1
        yMBR[2 * pCounter]=CoordToMBR(cY1, mbrType);
        yMBR2[2 * pCounter]=atof(cY1);
	//x2
        xMBR[2 * pCounter + 1]=CoordToMBR(cX2, mbrType);
        xMBR2[2 * pCounter + 1]=atof(cX2);
	//y2
        yMBR[2 * pCounter + 1]=CoordToMBR(cY2, mbrType);
        yMBR2[2 * pCounter + 1]=atof(cY2);
        if(xMBR[2 * pCounter] > xMBR[2 * pCounter +1])swapElements(xMBR, 2 * pCounter, 2 * pCounter +1);
        else if(xMBR[2 * pCounter] == xMBR[2 * pCounter + 1]){printf("\nMBR Error!\n");continue;}

        if(yMBR[2 * pCounter] > yMBR[2 * pCounter + 1])swapElements(yMBR, 2 * pCounter, 2 * pCounter + 1);
        else if(yMBR[2 * pCounter] == yMBR[2 * pCounter + 1]){printf("\nMBR Error!\n");continue;}


        if(xMBR2[2 * pCounter] > xMBR2[2 * pCounter +1])swapElements(xMBR2, 2 * pCounter, 2 * pCounter +1);
        else if(xMBR2[2 * pCounter] == xMBR2[2 * pCounter + 1]){printf("\nMBR Error!\n");continue;}

        if(yMBR2[2 * pCounter] > yMBR2[2 * pCounter + 1])swapElements(yMBR2, 2 * pCounter, 2 * pCounter + 1);
        else if(yMBR2[2 * pCounter] == yMBR2[2 * pCounter + 1]){printf("\nMBR Error!\n");continue;}

        fNum-=2;
        for(int k = 0; k < fNum + 1 ; k++){
            if( k != fNum){
              xCoords[ *bVNumSum + k] = polyBuff[k+2][0];
              yCoords[ *bVNumSum + k] = polyBuff[k+2][1];
            }
            else{
              xCoords[ *bVNumSum + k] = polyBuff[2][0];
              yCoords[ *bVNumSum + k] = polyBuff[2][1];
            }
            if( k != 0 && k != fNum )edgeSum += GetEuclideanDist( xCoords[ *bVNumSum + k - 1 ], yCoords[ *bVNumSum + k - 1 ], xCoords[ *bVNumSum + k ], yCoords[ *bVNumSum + k ] );
        }      
        *bVNumSum += fNum + 1;
        bVPSNum[pCounter] = *bVNumSum;
        bVNum[pCounter++] = fNum;
